 ...
 * and optionally save to a local file. Based on the class from Piwik analytics
 * 
 */
class uHttp
{
    ...
    public static function sendHttpRequest($aUrl, $timeout, $userAgent = null, $destinationPath = null, $followDepth = 0, $acceptLanguage = false, $byteRange = false, $getExtendedInfo = false, $httpMethod = 'GET')
    {
        ...
        if ($destinationPath) {
            ...
            if (($file = @fopen($destinationPath, 'wb')) === false || !is_resource($file)) {
                trigger_error(sprintf(__('Error while creating the file: %s'),$destinationPath),E_USER_ERROR);
                return "{ \"Status\": 500 }";
            }
        }
    ...
    {
        if ($followDepth > 5) {
            trigger_error(sprintf(__('Too many redirects (%s)', $followDepth)),E_USER_ERROR);
            return "{ \"Status\": 500 }";
        }
        ...
        if ($method == 'socket') {
            if (!self::isSocketEnabled()) {
                trigger_error(__('HTTP socket support is not enabled (php function fsockopen is not available)'),E_USER_ERROR);
                return "{ \"Status\": 500 }";
            }
            // initialization
            $url = @parse_url($aUrl);
            if ($url === false || !isset($url['scheme'])) {
                trigger_error(sprintf(__('Malformed URL: %s', $aUrl)),E_USER_ERROR);
                return "{ \"Status\": 500 }";
            }

            if ($url['scheme'] != 'http') {
                trigger_error(sprintf(__('Invalid protocol/scheme:  %s', $aUrl)),E_USER_ERROR);
                return "{ \"Status\": 500 }";
            }
            $host = $url['host'];
            ...
            if (($fsock = @fsockopen($connectHost, $connectPort, $errno, $errstr, $timeout)) === false || !is_resource($fsock)) {
                ...
                trigger_error(sprintf(__('Error while connecting to:  %s. Please try again later. %s', $host, $errstr)),E_USER_ERROR);
                return "{ \"Status\": 500 }";
            }
            ...
            while (!feof($fsock)) {
                ...
                if ($streamMetaData['timed_out']) {
                    ...
                    @fclose($fsock);
                    trigger_error(__('Timed out waiting for server response'),E_USER_ERROR);
                    return "{ \"Status\": 500 }";
                }
                ...
                if (!$status) {
                    ...
                    if (!preg_match('~^HTTP/(\d\.\d)\s+(\d+)(\s*.*)?~', $line, $m)) {
                        ...
                        @fclose($fsock);
                        trigger_error(sprintf(__('Expected server response code. Got %s'),rtrim($line,"\r\n")),E_USER_ERROR);
                        return "{ \"Status\": 500 }";
                    }
                ...
                if (preg_match('/^Location:\s*(.+)/', rtrim($line, "\r\n"), $m)) {
                    ...
                    if ($status < 300) {
                        trigger_error(sprintf(__('Unexpected redirect to Location:  %s for status code %s'),rtrim($line), $status),E_USER_ERROR);
            ...
            if (feof($fsock)
                ...
                trigger_error(__('Unexpected end of transmission'),E_USER_ERROR);
                return "{ \"Status\": 500 }";
            }
            ...
            if (!self::isCurlEnabled()) {
                ...
                trigger_error(__("CURL is not enabled in php.ini, but is being used."),E_USER_ERROR);
                return "{ \"Status\": 500 }";
            }
            $ch = @curl_init();
            ...
            if ($response === true) {
                ...
                if ($errstr != '') {
                    trigger_error(__('curl_exec: ') . $errstr);
                    return "{ \"Status\": 500 }";
                }
                $response = '';
            } else {
            ...
            trigger_error(__('Invalid request method: ') . $method);
            return "{ \"Status\": 500 }";
        }
        ...
        if (is_resource($file)) {
            ...
            if ((($contentLength > 0) && ($fileLength != $contentLength))
                ...
                trigger_error(sprintf(__('File size error: %s expected %s bytes; received %s bytes; saved %s bytes to file.'), $destinationPath , $contentLength , $fileLength , $fileSize),E_USER_ERROR);
                return "{ \"Status\": 500 }";
            }
            return true;
        }
     ...
     * is stored in the options table before starting a download. The expected
     ...
     * should continue to send requests to the server which will in turn call this method
     ...
     * {
     *     $outputPath = $directory . '/tmp/averybigfile.zip';
    ...
    public static function downloadChunk($url, $outputPath, $isContinuation)
    {
        ...
        if (!$isContinuation
            ...
            trigger_error(__('File already exists'),E_USER_ERROR);
            return "{ \"Status\": 500 }";
        }
        ...
        if (!$isContinuation) {
            $expectedFileSizeResult = uHttp::sendHttpRequest(
                $url,
                $timeout = 300,
                $userAgent = null,
                $destinationPath = null,
            ...
            $fp=fopen($directory."options.txt","w");
            fputs($fp,$expectedFileSize);
            fclose($fp);
        } else {
            $directory = dirname(__FILE__)."/_cache/";
            $expectedFileSize = file_get_contents($directory."options.txt");
            if ($expectedFileSize === false) { // sanity check
                trigger_error(__('Trying to continue a download that never started?'),E_USER_ERROR);
                return "{ \"Status\": 500 }";
            }
        }
        ...
        if ($existingSize >= $expectedFileSize) {
            return " { \"currentSize\": ".$existingSize.", \"expectedSize\": $expectedFileSize, \"Status\": 200 } ";
        }

        // download a chunk of the file
        $result = uHttp::sendHttpRequest(
            $url,
            $timeout = 300,
            $userAgent = null,
            $destinationPath = null,
            $followDepth = 0,
            $acceptLanguage = false,
            $byteRange = array($existingSize, min($existingSize + 1024 * 256 - 1, $expectedFileSize)),
            $getExtendedInfo = true
        );
        ...
        if ($result === false
            ...
            trigger_error( sprintf(__("Failed to download range '%s-%s' of file from url '%s'. Got result: %s"), $byteRange[0], $byteRange[1], $url, print_r($result, true)));
        ...
        return " { \"currentSize\": ".filesize($outputPath).", \"expectedSize\": $expectedFileSize, \"Status\": 200 } ";
    }
