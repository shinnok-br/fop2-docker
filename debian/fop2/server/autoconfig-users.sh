...
mysql -NB --raw -u $DBUSER -h $DBHOST $DBNAME -e "SELECT value FROM fop2settings" | while read LINEA
do
echo $LINEA
done
...
PERMQUERY="set @@group_concat_max_len=32768; SELECT CONCAT('perm=',fop2permissions.name,':',fop2permissions.permissions,':',IF(isnull(GROUP_CONCAT(device)),'',GROUP_CONCAT(device))) FROM fop2permissions LEFT JOIN fop2PermGroup ON fop2permissions.name=fop2PermGroup.name LEFT JOIN fop2GroupButton ON fop2GroupButton.group_name=name_group LEFT JOIN fop2buttons ON id_button=fop2buttons.id LEFT JOIN fop2contexts ON fop2permissions.context_id=fop2contexts.id WHERE %CONDITION% GROUP BY fop2permissions.name,fop2permissions.permissions" 
mysql -NB -u $DBUSER -h $DBHOST $DBNAME -e "${PERMQUERY/\%CONDITION\%/fop2contexts.context='$CONTEXT'}" | while read LINEA
do
echo $LINEA
done
...
# MYEXTEN=`echo $LINEA | cut -d= -f2 | cut -d:  -f1| sed -e 's/ /_/g' -e 's/\+//g'`
...
mysql -NB -u $DBUSER -h $DBHOST $DBNAME -e "${MAINQUERY/\%CONDITION\%/context='$CONTEXT'}" | while read LINEA
do
MYEXTEN=`echo $LINEA | cut -d: -f1 | cut -d\= -f2 | sed 's/\+//g'`
...
if [ $CONT == 0 ]; then
    ...
    mysql -NB -u $DBUSER -h $DBHOST $DBNAME -e "${PERMQUERY/\%CONDITION\%/1=1}" | while read LINEA
    do
    echo $LINEA
    done
    ...
    mysql -NB -u $DBUSER -h $DBHOST $DBNAME -e "${MAINQUERY/\%CONDITION\%/1=1}" | while read LINEA
    do
    MYEXTEN=`echo $LINEA | cut -d: -f1 | cut -d\= -f2 | sed 's/\+//g'`
    hash_find claves "${MYEXTEN}" 
    echo -n $LINEA | sed -e 's/EMPTYSECRET/'${MICLAVE}'/g' -e 's/ aapa/,/g' -e 's/: /:/g' -e 's/:,/:/g' -e 's/,$//g'
